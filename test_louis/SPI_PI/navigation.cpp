...
float ComputeCoefA(double long_start, double lat_start, double long_end, double lat_end){
	return (lat_end - lat_start)/(long_end - long_start);
}
...
float CapAlgorithm(){
	...
	Cap_Actuel = ComputeCoefA(CurrentPosition_Lon, CurrentPosition_Lat, PreviousPosition_Lon, PreviousPosition_Lat);
	...
	Cap_Destination = ComputeCoefA(DestinationPosition_Lon, DestinationPosition_Lat, PreviousPosition_Lon, PreviousPosition_Lat);
...
bool test_fin()
{
	float distance = DirectDistance(CurrentPosition_Lat, CurrentPosition_Lon, DestinationPosition_Lat, DestinationPosition_Lon);
	printf("distance = %f\n",distance);
	if(distance < 5)
		return(true);
	else
